...
| latitude           | 29.536702                 | 是       | 纬度                                                         |
| longitude          | 106.611035                | 是       | 经度                                                         |
| district           | 重庆市,重庆市,南岸区      | 是       | 地区                                                         |
...
使用这种形式部署需要配置用户信息和打卡信息配置文件，在项目`cron/info.json`目录下，形式为 **json**，根据打卡接口文档配置相应的参数值即可，

同时还需要配置打卡的时间点和打卡时间抖动范围
